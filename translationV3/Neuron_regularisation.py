...
class Layer:
    ...
    def apply_l1(self, grad_w, l1_lambda):
    #Apply L1 regularization to the weights only

        if l1_lambda > 0.0:
            grad_w = grad_w + l1_lambda * np.sign(self.w)
        return grad_w

    def apply_l2(self, grad_w, l2_lambda):
    # Apply L2 regularization (weight decay) to the weights only
        if l2_lambda > 0.0:
            grad_w = grad_w + l2_lambda * self.w
        return grad_w
...
class Neural_Network:
    ...
    def __init__(self, n_input_init, nb_n_l, activ, loss="MSE",l1_lambda=0.0, l2_lambda=0.0):
        '''
    ...
    def SGD(self, y, lr):
        ...
        for (i, neu) in enumerate(self.l):
            ...
            grad_w = neu.apply_l1(grad_w, self.l1_lambda)
            grad_w = neu.apply_l2(grad_w, self.l2_lambda)
