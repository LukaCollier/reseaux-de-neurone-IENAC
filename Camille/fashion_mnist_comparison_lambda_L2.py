...
# Différents lambda pour L2
lambda_values = [1e-5, 1e-4, 1e-3]  

regularizations = {
    f"L2_{lam}": {"name": "L2", "lambda": lam} for lam in lambda_values
}
...
for reg_label, reg_params in regularizations.items():
    ...
    # Création de l'objet régularisation L2
...
plt.title("Loss comparison for L2 with different lambda")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.yscale("log")
plt.grid(True, alpha=0.3)

colors = {f"L2_{lam}": col for lam, col in zip(lambda_values, ["blue", "green", "red"])}
